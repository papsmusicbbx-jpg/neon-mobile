...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; font-family: 'Courier New', Courier, monospace; touch-action: none; overscroll-behavior: none;}
        ...
        .mic-btn { background: var(--bg); color: var(--main); border: 2px solid var(--accent); padding: 10px 4px; text-align: center; font-weight: bold; font-size: 12px; border-radius: 6px; box-shadow: 0 0 10px var(--dark); transition: all 0.2s ease; flex-shrink: 0; cursor: pointer; }
    ...
    <script>
        ...
        function initSpeechEngine() {
            ...
            recognition.onend = function() {
                ...
                // AND we are NOT processing or speaking!
        ...
        function manualMicToggle() {
            if (!isConversing) {
                ...
                isConversing = true;
                // DO NOT START THE MIC YET! Send the greeting first so she can reply cleanly.
                sendMacro("Neon");
                
            } else {
                // User pressed the button again to manually put her to sleep
        ...
        function updateHudStateUI() {
            ...
            if (isProcessing) {
                ...
                statBox.innerHTML = `STATUS: BUSY<br>MIC: PROCESSING<br>SYS.VER: 5.7 PTT`;
            } else if (isConversing) {
                micBtn.innerText = `🎙️ LISTENING (${remainingSeconds}s)`;
                micBtn.className = "mic-btn conversing";
                statBox.innerHTML = `STATUS: ENGAGED<br>MIC: ACTIVE WINDOW<br>REMAINING: ${remainingSeconds}s<br>SYS.VER: 5.7 PTT`;
            } else {
                micBtn.innerText = "TAP TO WAKE N.E.O.N.";
                micBtn.className = "mic-btn";
                statBox.innerHTML = `STATUS: SLEEPING<br>MIC: OFFLINE<br>SYS.VER: 5.7 PTT`;
            }
        }
        ...
        function fallbackToDeviceSpeech(textResponse) {
            ...
            // Add a tiny 100ms delay to let the microphone fully release its audio lock before speaking
            setTimeout(() => {
